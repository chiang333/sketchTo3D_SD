...
from nerf.sd import encode_text_word_embedding
...
class Trainer(object):
    def __init__(self, 
        ...
        sketch_img = Image.open(opt.sketch_path).convert("RGB")
        ...
        sketch_image = sketch_image.resize((224, 224), resample=Image.BICUBIC)

        sketch_image = np.array(sketch_image).astype(np.uint8)
        self.sketch_image = (sketch_image / 127.5 - 1.0).astype(np.float32)
        self.sketch_image = torch.from_numpy(self.sketch_image).unsqueeze(0)
        ...
        if not opt.lora:
            ...
            text = opt.text.format(placeholder_string)
            print(text)
            text_input = self.guidance.tokenizer(text, max_length=self.guidance.tokenizer.model_max_length, padding="max_length",
                                           truncation=True, return_tensors="pt").input_ids.to(self.guidance.device)
            with torch.no_grad():
                clip_features = self.guidance.vision_encoder(
                    self.sketch_image.permute(0, 3, 1, 2).to(self.guidance.device)).last_hidden_state
                word_embeddings = self.guidance.inversion_adapter(clip_features.to(self.guidance.device))
                word_embeddings = word_embeddings.reshape((1, 8, -1))
                encoder_hidden_states = encode_text_word_embedding(self.guidance.text_encoder, text_input,
                                                                    word_embeddings,
            ...
            class LoraUnet(torch.nn.Module):
                def __init__(self):
                    ...
                    self.text_embeddings = encoder_hidden_states
    ...
    def prepare_text_embeddings(self):
        ...
        placeholder_string = " $" * 8
        text_input = self.opt.text.format(placeholder_string)

        if not self.opt.dir_text:
            self.text_z = self.guidance.get_text_embeds([text_input], [self.opt.negative])
        else:
            self.text_z = []
            for d in ['front', 'side', 'back', 'side', 'overhead', 'bottom']:
                # construct dir-encoded text
                text = f"{text_input}, {d} view"
                ...
                text_z = self.guidance.get_text_embeds([text], [negative_text], self.sketch_image)
